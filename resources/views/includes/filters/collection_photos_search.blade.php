<aside class="col-sm-6 offset-sm-3 col-lg-3 offset-lg-0">
    <form id="#" action="{{url('collections/'.$collection->id.'/photos')}}" method="GET" class="aside-form">
                    ...
                    <li class="aside-accordion__itm">
                        ...
                        <div class="aside-accordion__content">
                            @php
                                ...
                                $techniqueList = $searcher->prepareListSelect('techique_list', 27,'name_sort',$collection->id);
                            @endphp
                                ...
                                @foreach($techniqueList as $key => $technique)
                                    <dd class="aside-filter__itm">
                                        <input type="checkbox" class="aside-filter__checkbox"
                                               name="techniques[{{$key}}]" id="technique-{{$technique->item_id}}"
                                               value="{{$technique->item_id}}"
                                                {{is_array(old('techniques')) &&
                                                in_array($technique->item_id, old('techniques')) ? 'checked' : ''}}>
                                        <label for="technique-{{$technique->item_id}}">
                                            {{$technique->name_singular}}({{$technique->cnt}})
                                        </label>
                                    </dd>
                                @endforeach
                            </dl>
                    ...
                    <li class="aside-accordion__itm">
                        ...
                        <div class="aside-accordion__content">
                            <dl class="aside-filter list-style aside-filter--advanced">
                                <dt></dt>
                                @php
                                    $placesList = $searcher->prepareObjSelect(['singular' => 'place', 'plural' => 'places'], 27,'name_sort',false,$collection->id);
                                @endphp
